...
class TestG7FailureAfterEffectPreservesExistingBehavior:
    def test_failure_is_fail_closed_and_compensation_plan_covers_the_effect(
        ...
        # والعمليّةُ لم تُثبَّت ناجحةً: إعادةُ النداءِ لا تُرجِعُ «إعادةً» صامتةً
        # ولا نجاحًا مُدَّعى، بل تُغلِقُ صريحًا. وهذا سلوكُ 1F/1H القائم: الإذنُ
        # المُستهلَكُ لا يُعادُ استعمالُه، فالعمليّةُ الفاشلةُ تحتاجُ استردادًا
        # مُعلَنًا لا محاولةً صامتة.
        monkeypatch.undo()
        with pytest.raises(IdempotencyError):
            rt.allocate_budget("science", "400", "فشلٌ بعدَ الأثر", allocation_id="g7")
        assert _budget_of(rt, "science") == before, "أُنتِجَ أثرٌ ثانٍ في محاولةِ إعادة."
        assert len(authorizer.results) == 0, (
            "لم تُرجَع حصيلةٌ ناجحةٌ من نداءٍ فاشل — وهذا هو المتوقَّع."
        )
